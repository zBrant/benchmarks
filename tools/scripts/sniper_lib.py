...
import sys, json, os, re, subprocess, sniper_stats, sniper_config, json
...
def stats_process(config, results):
  ...
  stats['fs_to_cycles_cores'] = list(map(lambda f: f / 1e15, freq))
  ...
  if 'performance_model.elapsed_time' in stats and 'performance_model.idle_elapsed_time' in stats:
    stats['performance_model.nonidle_elapsed_time'] = [
      list(stats['performance_model.elapsed_time'])[c] - list(stats['performance_model.idle_elapsed_time'])[c]
      for c in range(ncores)
    ]
    stats['performance_model.idle_elapsed_time'] = [
      time0_end - time0_begin - list(stats['performance_model.nonidle_elapsed_time'])[c]
      for c in range(ncores)
    ]
...
def find_children(pid):
  ...
  for _pid in os.listdir('/proc'):
    try:
      ...
      continue # not a pid
    try:
      stat = open('/proc/%u/stat' % _pid).read()
    except IOError:
