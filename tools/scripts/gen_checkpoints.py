...
def parse_args():
    ...
    parser.add_argument('-r', '--root', type=str, default='.', help='<root-path> from results directory')
    parser.add_argument('-l', '--label', type=str, required=True, help='test label name')
    ...
    parser.add_argument('-i', '--input', type=str, default='ref', help='input size (e.g., ref, test)')
    ...
    parser.add_argument('-f', '--info', type=str, nargs='+', choices=COLUMN_OPTIONS, default=COLUMN_OPTIONS, help='information colums')
...
def get_args(args):
    # Updated path structure: {root}/{label}/{n_cores}/{benchmark}/{app}/{input}/{config}
    root_dir = f"{args.root}/{args.label}/{args.ncores}/{args.benchmark}"
    ...
    # Build config search path including input level
    configs = []
    if args.config:
        configs = args.config
    else:
        # Find configs from first app with input level
        if app_names:
            first_app_dir = f"{root_dir}/{app_names[0]}/{args.input}"
            if os.path.isdir(first_app_dir):
                configs = list(filter(lambda f: os.path.isdir(f"{first_app_dir}/{f}"), 
                                    sorted(os.listdir(first_app_dir), key=natural_keys)))
    
    test_name = '_' + args.label.replace('/', '-')
    out_file = f"results{test_name}_{args.benchmark}_n{args.ncores}.xlsx"
    
    return root_dir, args.benchmark, app_names, args.ncores, args.input, configs, args.info, out_file, args.err
...
def load_execution_data(root_dir, app_names, input_size, configs, infos):
    apps = []
    error = False
    for app_name in app_names:
        # Updated path: {root_dir}/{app_name}/{input_size}/{config}
        app_dir = f"{root_dir}/{app_name}/{input_size}"
        app_data = dict()
        for c in configs:
            config_path = f"{app_dir}/{c}"
            if os.path.isdir(config_path):
                app_data[c] = get_execution_data(config_path)
                if app_data[c]['error']:
                    error = True
            else:
...
def main():
    args = parse_args()
    root_dir, benchmark, app_names, ncores, input_size, configs, infos, out_file, err_file = get_args(args)

    apps = load_execution_data(root_dir, app_names, input_size, configs, infos)
...
# Example usage:
# ./gen_checkpoints.py -l default -n 4 -p cpu2017 -i ref -a perlbench_r gcc_r bwaves_r mcf_r